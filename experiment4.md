...
```python
protein（'UGC GAU GAA UGG GCU CGC UCC'）
```
...
```python
  键是用来描述一个字典中特定元素的名称，它是一个标签或者标识符，键必须是唯一的，这意味着在一个字典中，两个不同的元素不能有相同的键。
```

2. 在读取和写入字典时，需要使用默认值可以使用什么方法？

```python
在读取和写入字典时，可以使用字典的get()方法来设置默认值。get()方法接受一个键作为参数，并返回对应键的值。

当使用get()方法时，你可以在参数列表中传入一个默认值作为第二个参数。如果指定的键存在于字典中，get()方法将返回对应的值；如果键不存在于字典中，get()方法将返回指定的默认值。
```

3. Python中的while循环和for循环有什么区别？

```python
while循环：while循环会在满足指定条件时重复执行一段代码块。它的语法是：while 条件表达式：代码块。在每次循环开始之前，会先判断条件表达式的真假。只有当条件为真时，循环体中的代码块才会执行。当循环体执行完毕后，又会回到起始处重新判断条件，如果满足条件仍然为真，则继续执行循环体；如果条件为假，则结束循环。
for循环：for循环一般用于遍历（迭代）可迭代对象中的元素，例如列表、元组、字符串或字典等。它的语法是：for 变量 in 可迭代对象：代码块。在每次循环迭代时，会逐个获取可迭代对象中的元素，并将其赋值给变量，然后执行循环体中的代码块。当所有元素都被遍历完后，循环结束。
```
...
本次实验我学习了字典以及用户输入和while循环的相关内容。字典由键和值组成，具有一一对应的关系；用户的输入通过input()方式实现；while循环可以实现循环语句，直到条件不满足时停止循环。
